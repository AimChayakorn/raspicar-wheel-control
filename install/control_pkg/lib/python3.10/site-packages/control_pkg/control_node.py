...
class MinimalPublisher(Node):
    def __init__(self):
        ...
        # Create a task to read events asynchronously
        asyncio.get_event_loop().create_task(self.read_events())

    async def read_events(self):
        """Asynchronously read input events from the device."""
...
def spin_ros(executor, node):
    ...
    try:
        executor.spin()  # Just spin the executor
    finally:
...
def main(args=None):
    ...
    # Use a MultiThreadedExecutor for ROS
